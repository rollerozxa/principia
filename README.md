...
These instructions are for MSYS2, Please find the latest version of the 64-bit MSYS2 here: https://www.msys2.org/
...
The terminal will then ask you to shut down the MSYS2 runtime to the finish the update. Proceed with doing so, and then go to the start menu and start the "MSYS2 UCRT64" environment (icon with gold background).
...
Then start the build process by generating the build files using CMake and then
